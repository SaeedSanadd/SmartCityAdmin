...
export default function StatCard({ title, value, icon, bgClass }) {
    const accent = accentColors[bgClass] || { bar: "from-emerald-500 to-emerald-600", glow: "" };
    ...
    return (
        <div ref={ref} className={`relative overflow-hidden rounded-2xl bg-white hover-lift ${accent.glow} hover:shadow-lg`}>
            {/* Gradient accent bar */}
            <div className={`h-1 w-full bg-gradient-to-r ${accent.bar}`} />

            <div className="flex items-center gap-4 p-5">
                <div className={`h-12 w-12 rounded-xl flex items-center justify-center text-xl shrink-0 border ${bgClass || "bg-slate-50 border-slate-100 text-slate-600"}`}>
                    {icon}
                </div>
