...
export default function AnalyticsCharts({ reports }) {
    ...
    const categoryData = useMemo(() => {
        ...
        const othersCount = sorted.slice(3).reduce((sum, item) => sum + item.count, 0);
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-around gap-6 h-48">
                        ...
                        <div className="relative w-36 h-36 shrink-0">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                ...
                                <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">{t("report")}</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2.5 w-full max-w-[200px]">
                            {categoryData.map((item, idx) => {
                                const percent = Math.round((item.count / totalCategoriesCount) * 100);
                                return (
                                    <div key={idx} className="flex items-center justify-between text-xs">
                                        <div className="flex items-center gap-2 min-w-0">
                                            <div className={`h-2.5 w-2.5 rounded-full shrink-0 ${bgColors[idx % bgColors.length]}`} />
