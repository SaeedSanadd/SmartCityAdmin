...
export default function ReportsPage() {
    ...
    const [activeTab, setActiveTab] = useState("reports"); // 'reports' or 'bins'
    ...
    const exportToCSV = () => {
        ...
        const headers = ["Report ID", "Type", "Status", "City", "Address", "Date"];
        const csvRows = [
            headers.join(","),
            ...filtered.map(r => {
    ...
    return (
        <div className="space-y-5 animate-fadeInUp">
            {/* Tab selector */}
            <div className="flex border-b border-slate-200/80 mb-6 bg-white/40 p-1.5 rounded-2xl backdrop-blur-sm shadow-sm max-w-fit gap-1 animate-fadeInUp">
                <button
                    onClick={() => setActiveTab("reports")}
                    className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-250 cursor-pointer ${
                        activeTab === "reports"
                            ? "bg-primary text-white shadow-md hover:bg-primary-hover"
                            : "text-slate-500 hover:text-slate-800 hover:bg-white/50"
                    }`}
                >
                    {t("citizen_reports") || "Citizen Reports"}
                </button>
                <button
                ...
                <div className="space-y-5">
                    ...
                    <div className="rounded-2xl glass-card-strong overflow-hidden">
                        <div className="grid grid-cols-12 border-b border-slate-100/80 bg-slate-50/60 px-5 py-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                            <div className="col-span-7 text-start">{t("report")}</div>
